...
"""Console entry point to access quafu installation variables."""
...
def print_includes():
    ...
    dirs = [
        _ROOT / 'quafu' / 'include' / 'quafu',
        _ROOT / 'ccsrc' / 'include',
        _ROOT / 'ccsrc' / 'python' / 'core' / 'include',
        _ROOT / 'ccsrc' / 'python' / 'quafubackend' / 'include',
        _ROOT / 'ccsrc' / 'python' / 'simulator' / 'include',
    ]

    root = _ROOT / 'quafu' / 'lib' / 'quafu' / 'third_party'
    if root.exists():
...
def get_cmake_dir(as_string=True):
    """
    Return the path to the quafu CMake module directory.
    ...
    cmake_installed_path = Path(_ROOT, 'quafu', 'share', 'quafu', 'cmake')
    ...
    raise ImportError('quafu not installed, installation required to access the CMake files')
...
def print_bin_dir():
    """Print the bin directory of quafu."""
    bin_path = Path(_ROOT, "quafu", "bin")
    if bin_path.exists():
        print(str(bin_path))
        return
...
def print_abi():
    """Print the bin directory of quafu."""
    try:
        from quafu import quafubackend  # pylint: disable=import-outside-toplevel

        print(quafubackend.c.build_abi())  # pylint: disable=no-member
    except ImportError as exc:
        raise ImportError("quafu not installed.") from exc
...
def print_version():
    """Print quafu's version."""
    try:
        print(importlib_metadata.version('quafu'))
...
def main():
    ...
    group.add_argument(
        "--cmakedir",
        action="store_true",
        help=("Print the CMake module directory, ideal for setting either -Dquafu_ROOT or-Dquafu_DIR in " "CMake."),
    )
    group.add_argument(
        "--includes",
        action="store_true",
        help="Include flags for quafu",
    )
    group.add_argument(
        "--tempdir",
        action="store_true",
    ...
    group.add_argument(
        "--version",
        action="store_true",
        help="Print out quafu's version",
    )
    group.add_argument(
        "--bin",
        action="store_true",
        help="Print bin directory of quafu",
    )
    group.add_argument(
        "--abi",
        action="store_true",
        help="Print abi of quafu backend",
    )
