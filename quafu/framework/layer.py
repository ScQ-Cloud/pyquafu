...
from .operations import QUAFUAnsatzOnlyOps, QUAFUN2AnsatzOnlyOps, QUAFUN2Ops, QUAFUOps, QRamVecOps


class QUAFULayer(nn.Cell):  # pylint: disable=too-few-public-methods
    """
    ...
    Examples:
        ...
        >>> from quafu.framework import QUAFULayer
    ...
    def __init__(self, expectation_with_grad, weight='normal'):
        """Initialize a QUAFULayer object."""
        super().__init__()
        self.evolution = QUAFUOps(expectation_with_grad)
    ...
    def construct(self, arg):
        """Construct a QUAFULayer node."""
        return self.evolution(arg, self.weight)


class QUAFUN2Layer(nn.Cell):  # pylint: disable=too-few-public-methods
    """
    quafu trainable layer.
    ...
    Examples:
        ...
        >>> from quafu.core.circuit import Circuit
        >>> from quafu.core.operators import Hamiltonian, QubitOperator
        >>> from quafu.framework import QUAFUN2Layer
        >>> from quafu.simulator import Simulator
        >>> ms.set_seed(42)
        >>> ms.set_context(mode=ms.PYNATIVE_MODE, device_target="CPU")
        >>> enc = Circuit().ry('a', 0).as_encoder()
    ...
    def __init__(self, expectation_with_grad, weight='normal'):
        """Initialize a QUAFUN2Layer object."""
        super().__init__()
        self.evolution = QUAFUN2Ops(expectation_with_grad)
        weight_size = len(self.evolution.expectation_with_grad.ansatz_params_name)
        if isinstance(weight, ms.Tensor):
            if weight.ndim != 1 or weight.shape[0] != weight_size:
    ...
    def construct(self, arg):
        """Construct a QUAFUN2Layer node."""
        return self.evolution(arg, self.weight)


class QUAFUAnsatzOnlyLayer(nn.Cell):  # pylint: disable=too-few-public-methods
    """
    quafu trainable layer.
    ...
    Examples:
        ...
        >>> net =  QUAFUAnsatzOnlyLayer(grad_ops)
        >>> opti = ms.nn.Adam(net.trainable_params(), learning_rate=0.1)
        >>> train_net = ms.nn.TrainOneStepCell(net, opti)
        >>> for i in range(100):
        ...     train_net()
    ...
    def __init__(self, expectation_with_grad, weight='normal'):
        """Initialize a QUAFUAnsatzOnlyLayer object."""
        super().__init__()
        self.evolution = QUAFUAnsatzOnlyOps(expectation_with_grad)
        weight_size = len(self.evolution.expectation_with_grad.ansatz_params_name)
        if isinstance(weight, ms.Tensor):
            if weight.ndim != 1 or weight.shape[0] != weight_size:
                raise ValueError(f"Weight init shape error, required ({weight_size}, ), but get f{weight.shape}.")
    ...
    def construct(self):
        """Construct a QUAFUAnsatzOnlyLayer node."""
        return self.evolution(self.weight)


class QUAFUN2AnsatzOnlyLayer(nn.Cell):  # pylint: disable=too-few-public-methods
    """
    quafu trainable layer.
    ...
    Examples:
        ...
        >>> from quafu.core.circuit import Circuit
        >>> from quafu.core.operators import Hamiltonian, QubitOperator
        >>> from quafu.framework import QUAFUN2AnsatzOnlyLayer
        >>> from quafu.simulator import Simulator
        >>> ms.set_seed(43)
        >>> ms.set_context(mode=ms.PYNATIVE_MODE, device_target="CPU")
        >>> circ = Circuit().ry('a', 0).h(0).rx('b', 0)
        ...
        >>> sim = Simulator('quafuvector', 1)
        >>> grad_ops = sim.get_expectation_with_grad(ham, circ)
        >>> net =  QUAFUN2AnsatzOnlyLayer(grad_ops)
        >>> opti = ms.nn.Adam(net.trainable_params(), learning_rate=0.1)
        >>> train_net = ms.nn.TrainOneStepCell(net, opti)
        >>> for i in range(100):
        ...     train_net()
    ...
    def __init__(self, expectation_with_grad, weight='normal'):
        """Initialize a QUAFUN2AnsatzOnlyLayer object."""
        super().__init__()
        self.evolution = QUAFUN2AnsatzOnlyOps(expectation_with_grad)
        weight_size = len(self.evolution.expectation_with_grad.ansatz_params_name)
        if isinstance(weight, ms.Tensor):
            if weight.ndim != 1 or weight.shape[0] != weight_size:
                raise ValueError(f"Weight init shape error, required ({weight_size}, ), but get f{weight.shape}.")
    ...
    def construct(self):
        """Construct a QUAFUN2AnsatzOnlyLayer node."""
        return self.evolution(self.weight)
...
class QRamVecLayer(nn.Cell):  # pylint: disable=too-few-public-methods
    ...
    Examples:
        ...
        >>> from quafu.core.circuit import Circuit
        >>> from quafu.core.operators import Hamiltonian, QubitOperator
        >>> from quafu.framework import QRamVecLayer
        >>> from quafu.simulator import Simulator
        >>> ms.set_seed(42)
        >>> ms.set_context(mode=ms.PYNATIVE_MODE, device_target="CPU")
