...
from quafu.core.gates import BarrierGate, BasicGate, Measure, MeasureResult
from quafu.core.gates.basicgate import MultiParamsGate
from quafu.core.operators import Hamiltonian
from quafu.core.parameterresolver import ParameterResolver
from quafu.dtype import complex128
from quafu.dtype.dtype import quafu_complex_number_type
...
class QUAFUSim(BackendBase):
    ...
    def __str__(self):
        ...
        if self.name.startswith('quafuvector'):
    ...
    def apply_hamiltonian(self, hamiltonian: Hamiltonian):
        """Apply a hamiltonian."""
        if not quafu.is_same_precision(self.dtype, hamiltonian.dtype):
            raise TypeError(
                f"Data type of {self.name} simulator is {quafu.precision_str(self.dtype)} ({self.dtype}), "
                f"but given hamiltonian is {quafu.precision_str(hamiltonian.dtype)} ({hamiltonian.dtype}). "
                f"Please convert given hamiltonian to {quafu.precision_str(self.dtype)} "
                f"({quafu.to_precision_like(hamiltonian.dtype, self.dtype)})."
            )
    ...
    def astype(self, dtype, seed):
        ...
        _check_quafu_type(dtype)
        if dtype not in quafu_complex_number_type:
            raise TypeError(f"dtype should be complex type, available types are {quafu_complex_number_type}")
        sim = QUAFUSim(getattr(self.sim, str(dtype).rsplit('.', maxsplit=1)[-1])(seed), self.n_qubits, internal=True)
    ...
    def copy(self) -> "BackendBase":
        """Copy a simulator."""
        sim = QUAFUSim(self.name, self.n_qubits, self.seed)
    ...
    def get_expectation(
        ...
        if self.name == 'quafumatrix':
            raise NotImplementedError("Non hermitian case for get_expectation not implement for quafumatrix yet.")
    ...
    def get_expectation_with_grad(  # pylint: disable=R0912,R0913,R0914,R0915
        ...
        for i, ham in enumerate(hams):
            if not quafu.is_same_precision(self.dtype, ham.dtype):
                raise TypeError(
                    f"Data type of {self.name} simulator is {quafu.precision_str(self.dtype)} ({self.dtype}),"
                    f" but {i}th hamiltonian is {quafu.precision_str(ham.dtype)} ({ham.dtype}). "
                    f"Please convert {i}th hamiltonian to {quafu.precision_str(self.dtype)} "
                    f"({quafu.to_precision_like(ham.dtype, self.dtype)})."
                )
        for h_tmp in hams:
        ...
        if circ_right.is_noise_circuit and "quafuvector" in self.name:
            ...
            pr_shift = True
        if pr_shift and not self.name.startswith('quafuvector'):
        ...
        if circ_left is not None:
            ...
            non_hermitian = True
        if simulator_left is not None:
            _check_input_type("simulator_left", QUAFUSim, simulator_left)
        ...
        def grad_ops(*inputs_):
            ...
            elif circ_right.is_noise_circuit and self.name.startswith('quafumatrix'):
    ...
    def get_qs(self, ket=False) -> np.ndarray:
        """Get quantum state of quafuvector simulator."""
    ...
    def reset(self):
        """Reset quafu simulator to quantum zero state."""
    ...
    def set_qs(self, quantum_state: np.ndarray):
        ...
        Args:
            quantum_state: Input quantum state as numpy array. For quafumatrix simulator,
                can be either 1D vector or 2D density matrix. For quafuvector simulator,
                must be 1D vector.
        """
        try:
    ...
    def get_partial_trace(self, obj_qubits) -> np.ndarray:
        ...
        if not self.name.startswith('quafumatrix'):
