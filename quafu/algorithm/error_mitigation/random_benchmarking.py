...
def query_single_qubit_clifford_elem(idx: int) -> Simulator:
    ...
    Examples:
        >>> from quafu.algorithm.error_mitigation import query_single_qubit_clifford_elem
        >>> from quafu.simulator import decompose_stabilizer
    ...
    clifford = Stabilizer(_quafu_vector.stabilizer.query_single_qubit_clifford_elem(idx), n_qubits=1, internal=True)
...
def query_double_qubits_clifford_elem(idx: int) -> Simulator:
    ...
    Examples:
        >>> from quafu.algorithm.error_mitigation import query_double_qubits_clifford_elem
        >>> from quafu.simulator import decompose_stabilizer
    ...
    _check_int_type('idx', idx)
    _check_value_should_not_less("idx", 0, idx)
    clifford = Stabilizer(_quafu_vector.stabilizer.query_double_qubits_clifford_elem(idx), n_qubits=2, internal=True)
...
def generate_single_qubit_rb_circ(length: int, seed: int = None) -> Circuit:
    ...
    Examples:
        >>> import numpy as np
        >>> from quafu.algorithm.error_mitigation import generate_single_qubit_rb_circ
    ...
    stabs = _quafu_vector.stabilizer.generate_single_qubit_rb_circ(length, seed)
    circ = Circuit()
    for stab in stabs:
        circ.barrier()
...
def generate_double_qubits_rb_circ(length: int, seed: int = None) -> Circuit:
    ...
    Examples:
        >>> import numpy as np
        >>> from quafu.algorithm.error_mitigation import generate_double_qubits_rb_circ
    ...
    stabs = _quafu_vector.stabilizer.generate_double_qubits_rb_circ(length, seed)
    circ = Circuit()
    for stab in stabs:
        circ.barrier()
