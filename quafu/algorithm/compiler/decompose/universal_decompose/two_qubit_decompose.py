...
from quafu.core.gates import QuantumGate
...
def tensor_product_decompose(gate: QuantumGate, return_u3: bool = True) -> Circuit:
    ...
    Examples:
        >>> import numpy as np
        >>> import quafu
        >>> from quafu.algorithm.compiler.decompose import tensor_product_decompose
        >>> g = quafu.UnivMathGate('XY', np.kron(quafu.X.matrix(), quafu.Y.matrix())).on([0, 1])
        >>> print(quafu.Circuit() + g)
              ┏━━━━┓
        q0: ──┨    ┠───
              ┃    ┃
              ┃ XY ┃
...
def abc_decompose(gate: QuantumGate, return_u3: bool = True) -> Circuit:
    ...
    Examples:
        >>> import quafu
        >>> from quafu.algorithm.compiler.decompose import abc_decompose
        >>> from scipy.stats import unitary_group
        >>> g = quafu.UnivMathGate('U', unitary_group.rvs(2, random_state=123)).on(1, 0)
        >>> print(quafu.Circuit() + g)
        q0: ────■─────
                ┃
              ┏━┻━┓
...
def kak_decompose(gate: QuantumGate, return_u3: bool = True) -> Circuit:
    ...
    Examples:
        >>> import quafu
        >>> from quafu.algorithm.compiler.decompose import kak_decompose
        >>> from scipy.stats import unitary_group
        >>> g = quafu.UnivMathGate('U', unitary_group.rvs(4, random_state=123)).on([0, 1])
        >>> print(quafu.Circuit() + g)
              ┏━━━┓
        q0: ──┨   ┠───
              ┃   ┃
