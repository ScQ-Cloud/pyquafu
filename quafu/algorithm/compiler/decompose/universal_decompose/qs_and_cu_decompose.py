...
from quafu.core.gates import BarrierGate, QuantumGate
...
def cu_decompose(gate: QuantumGate, with_barrier: bool = False) -> Circuit:
    ...
    Examples:
        >>> import quafu
        >>> from quafu.algorithm.compiler import cu_decompose
        ...
        >>> circ = cu_decompose(g)
        >>> num_cnot = len([g for g in circ if isinstance(g, quafu.XGate) and len(g.ctrl_qubits)==1])
...
def qs_decompose(gate: QuantumGate, with_barrier: bool = False) -> Circuit:
    ...
    Examples:
        >>> import quafu
        >>> from quafu.algorithm.compiler.decompose import qs_decompose
        >>> from scipy.stats import unitary_group
        >>> tqs = [1,2,3,6] # arbitrary qubit index order is OK
        >>> n = len(tqs) # qubit number
        >>> u = unitary_group.rvs(2 ** n, random_state=123)
        >>> g = quafu.UnivMathGate('U', u).on(tqs)
        >>> circ = qs_decompose(g)
        >>> num_cnot =  len([g for g in circ if isinstance(g, quafu.XGate) and len(g.ctrl_qubits)==1])
        >>> print('total gate number: {}, CNOT number: {}'.format(len(circ), num_cnot))
