...
class CircuitPlotManager:
    ...
    def __init__(self, qc):
        ...
        self.q_label = {
            y: r'$|q_{%d}\rangle$' % i
            for i, y in self.used_qbit_y.items()
        }
        self.c_label = {
            self.used_qbit_y[iq]: r'c_{%d}' % ic
            for iq, ic in qc.measures.items()
        }
    ...
    def __call__(self,
        ...
        if title is not None:
            title = Text((self.xs[0] + self.xs[-1]) / 2,
                         -0.8,
                         title,
                         size=30,
                         ha='center',
    ...
    def _process_ins(self, ins: Instruction, append: bool = True):
        ...
        if name == 'barrier':
            ...
            self._ctrl_wire_points.append([[depth, ins.pos[0]],
                                           [depth, ins.pos[1]]])
    ...
    def _proc_ctrl(self, depth, ins: ControlledGate, ctrl_type: bool = True):
        ...
        if ins.ct_nums == (1, 1, 2) or name in mc_gate_names:
            ...
            self._swap_points += [[depth, self.used_qbit_y[p]]
                                  for p in ins.targs]
        elif name == 'ccx':
            ...
            assert isinstance(
                ins,
                ControlledU), f'unknown gate: {name}, {ins.__class__.__name__}'
