...
from quafu.transpile.graph.circuitgraph import draw_graph
...
class CouplingGraph:
    ...
    def __init__(self, coupling_list=None):
        ...
        if coupling_list is not None:
            # Sort nodes
            nodeset = sorted(set(item for sublist in coupling_list for item in sublist[0:2]))
    ...
    def shortest_undirected_path(self, source_qubit, target_qubit):
        ...
        if not path:
            raise ValueError("Error: Nodes %s and %s are not connected." % (str(source_qubit), str(target_qubit)))
        return path
                ...
                for n2 in range(n1 + 1, nodes):
                    ...
                    if len(swap_path) == 2:  # not need swap
                        ...
                        for i in range(len(swap_path) - 2):  # SWAP gates need to be inserted
                            min_f = min(np.log(self.edge_dict[(swap_path[i], swap_path[i + 1])]),
                                        np.log(self.edge_dict[(swap_path[i + 1], swap_path[i])]))
                            max_f = max(np.log(self.edge_dict[(swap_path[i], swap_path[i + 1])]),
                                        np.log(self.edge_dict[(swap_path[i + 1], swap_path[i])]))
                    ...
                    if len(swap_path) == 2:  # not need swap
                        ...
                        for i in range(len(swap_path) - 2):  # SWAP gates need to be inserted
                            ...
                            max_f = max(np.log(self.edge_dict[(swap_path[i], swap_path[i + 1])]),
                                        np.log(self.edge_dict[(swap_path[i + 1], swap_path[i])]))
                            fidelity += 2 * max_f + min_f
                            # fidelity += 2 * np.log(self.edge_dict[(swap_path[i], swap_path[i + 1])]) + \
                            #             np.log(self.edge_dict[(swap_path[i + 1], swap_path[i])])
                        fidelity += np.log(self.edge_dict[(swap_path[-1], swap_path[-2])])
