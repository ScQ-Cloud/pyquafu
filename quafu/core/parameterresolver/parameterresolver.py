...
import quafu
from quafu._math.pr import ParameterResolver as ParameterResolver_
from quafu._math.tensor import from_numpy
from quafu.dtype.dtype import quafu_complex_number_type, str_dtype_map
from quafu.utils.string_utils import join_without_empty, string_expression
...
class ParameterResolver(ParameterResolver_):
    ...
    Args:
        ...
        const (number.Number): the constant part of this parameter resolver.
            Default: ``None``.
        dtype (quafu.dtype): the data type of this parameter resolver. Default: ``None``.
        ...
        if internal:
            ...
            if isinstance(data, str):
                if dtype is None:
                    ...
                    if const is not None:
                        if isinstance(const, numbers.Number) and not isinstance(const, numbers.Real):
                            dtype = quafu.complex128
                if const is None:
                    const = from_numpy(np.array([0.0], dtype=quafu.to_np_type(dtype)))
                else:
                    const = from_numpy(np.array([const], dtype=quafu.to_np_type(dtype)))
                ParameterResolver_.__init__(self, data, const, dtype)  # PR('a'[, 1.0, quafu.float64])
                ...
                if const is None:
                    ...
                    const = from_numpy(np.array([const], dtype=quafu.to_np_type(dtype)))
                # PR({'a': 1.0}[, 2.0, quafu.float64])
                ParameterResolver_.__init__(
                    self,
                    {i: from_numpy(np.array([j], dtype=quafu.to_np_type(dtype))) for i, j in data.items()},
                    const,
                    dtype,
                )
                ...
                if dtype is None:
                    ...
                    if isinstance(data, numbers.Number) and not isinstance(data, numbers.Real):
                        dtype = quafu.complex128
                ParameterResolver_.__init__(
                    self, from_numpy(np.array([data], dtype=quafu.to_np_type(dtype)))
                )  # PR(1.0[, quafu.float64])
            elif data is None:
    ...
    def is_complex(self) -> bool:
        ...
        return self.dtype in quafu_complex_number_type
    ...
    def params_name(self) -> typing.List[str]:
        ...
        Examples:
            >>> from quafu import ParameterResolver
    ...
    def loads(strs: str) -> "ParameterResolver":
        ...
        if dtype in quafu_complex_number_type:
    ...
    def astype(self, dtype) -> "ParameterResolver":
        ...
        Args:
            dtype (quafu.dtype): new data type of parameter resolver you want.
    ...
    def conjugate(self) -> "ParameterResolver":
        ...
        Examples:
            >>> import quafu
            >>> from quafu.core.parameterresolver import ParameterResolver as PR
            >>> pr = PR({'a' : 1, 'b': 1j}, dtype=quafu.complex128)
    ...
    def dumps(self, indent=4) -> str:
        ...
        Note:
            ...
            quafu.float32 and quafu.complex64 type is not serializable.
        ...
        if quafu.is_single_precision(self.dtype):
            double_version = self.astype(quafu.to_double_precision(self.dtype))
        else:
            double_version = self
    ...
    def to_real_obj(self) -> "ParameterResolver":
        ...
        return self.astype(quafu.to_real_type(self.dtype))
