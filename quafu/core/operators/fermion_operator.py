...
import quafu
from quafu._math.ops import FermionOperator as FermionOperator_
from quafu._math.ops import f_term_value
from quafu.core.operators._term_value import TermValue
from quafu.core.parameterresolver import ParameterResolver, PRConvertible
from quafu.dtype.dtype import str_dtype_map
from quafu.quafubackend import EQ_TOLERANCE
...
class FermionOperator(FermionOperator_):
    ...
    def is_complex(self) -> bool:
        ...
        return self.dtype in (quafu.complex128, quafu.complex64)
    ...
    def from_openfermion(of_ops) -> "FermionOperator":
        """
        Convert openfermion fermion operator to quafu format.

        Args:
            of_ops (openfermion.FermionOperator): fermion operator from openfermion.

        Returns:
            FermionOperator, fermion operator from quafu.
        """
    ...
    def astype(self, dtype) -> "FermionOperator":
        ...
        Args:
            dtype (quafu.dtype): new data type of fermion operator.
    ...
    def cast_complex(self) -> "FermionOperator":
        ...
        new_type = self.dtype
        if new_type == quafu.float32:
            new_type = quafu.complex64
        elif new_type == quafu.float64:
            new_type = quafu.complex128
    ...
    def matrix(self, n_qubits: int = None, pr=None):  # pylint: disable=too-many-branches
        ...
        np_type = quafu.to_np_type(self.dtype)
        ops = self
