...
class Hamiltonian:
    ...
    def __init__(self, hamiltonian, dtype=None):
        ...
        if isinstance(hamiltonian, sp.csr_matrix):
            if dtype is None:
                dtype = quafu.to_quafu_type(hamiltonian.dtype)
            else:
                if dtype != quafu.to_quafu_type(hamiltonian.dtype):
                    hamiltonian = hamiltonian.astype(quafu.to_np_type(dtype))
    ...
    def astype(self, dtype):
        ...
        Args:
            dtype (quafu.dtype): the new type of hamiltonian.
        """
        if self.how_to == HowTo.FRONTEND:
    ...
    def get_cpp_obj(self, hermitian=False):
        ...
        """
        if quafu.is_double_precision(self.dtype):
