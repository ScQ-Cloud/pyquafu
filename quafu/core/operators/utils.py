...
def count_qubits(operator):
    ...
    Examples:
        >>> from quafu.core.operators import QubitOperator,FermionOperator, count_qubits
...
def commutator(left_operator, right_operator):
    ...
    Examples:
        >>> from quafu.core.operators import QubitOperator, FermionOperator, commutator
...
def normal_ordered(fermion_operator):
    ...
    Examples:
        >>> from quafu.core.operators import FermionOperator, normal_ordered
...
def number_operator(n_modes=None, mode=None, coefficient=1.0):
    ...
    Examples:
        >>> from quafu.core.operators import FermionOperator, number_operator
        >>> n_mode = 3
...
def hermitian_conjugated(operator):
    ...
    Examples:
        >>> from quafu.core.operators import QubitOperator, hermitian_conjugated
...
def up_index(index):
    ...
    Examples:
        >>> from quafu.core.operators import up_index
        >>> up_index(1)
        2
    """
    return 2 * index
...
def down_index(index):
    ...
    Examples:
        >>> from quafu.core.operators import down_index
        >>> down_index(1)
        3
    """
    return 2 * index + 1
...
def sz_operator(n_spatial_orbitals):
    ...
    Examples:
        >>> from quafu.core.operators import sz_operator
        >>> sz_operator(3)
        1/2 [0^ 0] +
        -1/2 [1^ 1] +
        1/2 [2^ 2] +
...
def ground_state_of_sum_zz(ops: QubitOperator, sim='quafuvector') -> float:
    """
    ...
    Args:
        ...
        sim (str): use which simulator to do calculation. Currently, we support
            ``'quafuvector'`` and ``'quafuvector_gpu'``. Default: ``'quafuvector'``.

    Returns:
        float, the ground state energy of given qubit operator.

    Examples:
        >>> from quafu.core.operators import ground_state_of_sum_zz, QubitOperator
    ...
    if sim.startswith('quafumatrix'):
        raise ValueError("quafumatrix simulator not support this method yet.")
