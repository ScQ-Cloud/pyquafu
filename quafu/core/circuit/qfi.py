...
def _qfi_matrix_base(circuit: Circuit, which_part='both', backend='quafuvector'):
...
def qfi(circuit: Circuit, backend='quafuvector'):
    r"""
    ...
    Args:
        ...
        backend (str): A supported simulator backend. Please refer description
            of :class:`~.simulator.Simulator`. Default: ``'quafuvector'``.
    ...
    Examples:
        >>> import numpy as np
        >>> from quafucircuit import qfi, Circuit
...
def partial_psi_partial_psi(circuit: Circuit, backend='quafuvector'):
    r"""
    ...
    Examples:
        >>> import numpy as np
        >>> from quafucircuit import partial_psi_partial_psi, Circuit
        >>> circ = Circuit().rx('a', 0).ry('b', 0).rz('c', 0)
...
def partial_psi_psi(circuit: Circuit, backend='quafuvector'):
    r"""
    Calculate the following value of the given parameterized quantum circuit.
    ...
    Examples:
        >>> import numpy as np
        >>> from quafucircuit import partial_psi_psi, Circuit
        >>> circ = Circuit().rx('a', 0).ry('b', 0).rz('c', 0)
