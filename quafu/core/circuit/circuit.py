...
from quafu.utils.quantifiers import quantifier_selector
...
from .. import gates as quafu_gates
...
def apply(circuit_fn, qubits):
    ...
    Examples:
        ...
        >>> from quafu.core.circuit import apply
        >>> u1 = qft([0, 1])
        >>> u2 = apply(u1, [1, 0])
        >>> u3 = apply(qft, [1, 0])
        >>> u3 = u3([0, 1])
...
GateSeq = List[quafu_gates.BasicGate]
...
class Circuit(list):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    Examples:
        ...
        >>> from quafu.core.gates import RX, X
        >>> circuit1 = Circuit()
        >>> circuit1 += RX('a').on(0)
        >>> circuit1 *= 2
        >>> circuit1
    ...
    def append(self, gate):
        ...
        if isinstance(gate, quafu_gates.Measure):
            self.all_measures.collect_only_one(gate, f'measure key {gate.key} already exist.')
        if isinstance(gate, quafu_gates.NoiseGate):
    ...
    def __eq__(self, other: "Circuit"):
        ...
        from quafu.core.gates import BarrierGate
    ...
    def __add__(self, gates):
        ...
        out = Circuit()
        out.extend(self)
        if isinstance(gates, quafu_gates.BasicGate):
            out.append(gates)
        else:
            out.extend(gates)
        return out
    ...
    def __setitem__(self, k, v):
        ...
        if isinstance(old_v, quafu_gates.Measure):
            self.all_measures.delete(old_v)
        if isinstance(old_v, quafu_gates.NoiseGate):
        ...
        if isinstance(v, quafu_gates.Measure):
            self.all_measures.collect_only_one(v, f'measure key {v.key} already exist.')
        if isinstance(v, quafu_gates.NoiseGate):
    ...
    def insert(self, index, gates):
        ...
        if isinstance(gates, quafu_gates.BasicGate):
            ...
            if isinstance(gates, quafu_gates.Measure):
    ...
    def __repr__(self):
        ...
        from quafu.io.display._config import _text_circ_config
        from quafu.io.display.circuit_text_drawer import rich_circuit
        from quafu.io.display.circuit_text_drawer_helper import Monitor
    ...
    def depth(self, with_single: bool = True, with_barrier: bool = False) -> int:
        ...
        Examples:
            ...
            2
        """
        from quafu.core.gates import NoiseGate, BarrierGate, Measure
    ...
    def summary(self, show=True):
        ...
        Examples:
            >>> from quafu.core.circuit import Circuit
            >>> from quafu.core.gates import RX, H
        ...
        measure_gate = 0
        for gate in self:
            if isinstance(gate, quafu_gates.BarrierGate):
                barrier += 1
                continue
            if isinstance(gate, quafu_gates.NoiseGate):
                noise_channel += 1
    ...
    def hermitian(self):
        ...
        Examples:
            >>> from quafu.core.circuit import Circuit
            >>> from quafu.core.gates import RX
    ...
    def encoder_params_name(self):
        ...
        Examples:
            >>> from quafu.core.gates import RX, RY
            >>> from quafu.core.circuit import Circuit
    ...
    def matrix(self, pr=None, big_end=False, backend='quafuvector', seed=None, dtype=None):
        """
        ...
        Args:
            ...
            backend (str): The backend to do simulation. Default: 'quafuvector'.
            seed (int): The random to generate circuit matrix, if the circuit has noise channel.
            dtype (quafu.dtype): data type of simulator. Default: 'None'.
        ...
        if backend.startswith('quafumatrix'):
            raise ValueError("quafumatrix backend not support to get circuit matrix.")
        # pylint: disable=import-outside-toplevel,cyclic-import
    ...
    def apply_value(self, pr):
        ...
        Examples:
            >>> from quafu.core.gates import X, RX
            >>> from quafu.core.circuit import Circuit
            >>> circuit = Circuit()
            >>> circuit += X.on(0)
            >>> circuit += RX({'a': 2}).on(0)
    ...
    def remove_barrier(self):
        ...
        circ = Circuit()
        for gate in self:
            if not isinstance(gate, quafu_gates.BarrierGate):
                circ += gate
        return circ
    ...
    def remove_measure(self):
        ...
        circ = Circuit()
        for gate in self:
            if not isinstance(gate, quafu_gates.Measure):
                circ += gate
        return circ
    ...
    def remove_measure_on_qubits(self, qubits):
        ...
        Examples:
            ...
            >>> from quafu.core.gates import H, Measure
        ...
        circ = Circuit()
        for gate in self:
            if isinstance(gate, quafu_gates.Measure) and gate.obj_qubits[0] in qubits:
                continue
            circ += gate
        return circ
    ...
    def get_cpp_obj(self, hermitian=False):
        ...
        if hermitian:
            if not self.has_herm_cpp_obj:
                ...
                self.herm_cpp_obj = [
                    i.get_cpp_obj() for i in self.hermitian() if not isinstance(i, quafu_gates.BarrierGate)
                ]
        ...
        if not self.has_cpp_obj:
            ...
            self.cpp_obj = [i.get_cpp_obj() for i in self if not isinstance(i, quafu_gates.BarrierGate)]
    ...
    def h(self, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.H.on(obj_qubits, ctrl_qubits))
        return self
    ...
    def x(self, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.X.on(obj_qubits, ctrl_qubits))
        return self
    ...
    def y(self, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.Y.on(obj_qubits, ctrl_qubits))
        return self
    ...
    def z(self, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.Z.on(obj_qubits, ctrl_qubits))
        return self
    ...
    def s(self, obj_qubits, ctrl_qubits=None, hermitian=False):
        ...
        """
        gate = quafu_gates.S.on(obj_qubits, ctrl_qubits)
        if hermitian:
            gate = gate.hermitian()
        self.append(gate)
        return self
    ...
    def t(self, obj_qubits, ctrl_qubits=None, hermitian=False):
        ...
        Args:
            ...
            hermitian (bool): Whether use the hermitian conjugated version. Default: ``False``.
        """
        gate = quafu_gates.T.on(obj_qubits, ctrl_qubits)
        if hermitian:
            gate = gate.hermitian()
        self.append(gate)
        return self
    ...
    def to_openqasm(self, file_name: Optional[str] = None, version: str = '2.0') -> str:
        """
        Convert a quafu circuit to OpenQASM format string and optionally save to a file.
        ...
        """
        # pylint: disable=import-outside-toplevel
        from quafu.io.qasm import OpenQASM
    ...
    def to_hiqasm(self, file_name: Optional[str] = None, version: str = '0.1') -> str:
        """
        Convert a quafu circuit to HiQASM format string and optionally save to a file.
        ...
        """
        # pylint: disable=import-outside-toplevel
        from quafu.io.qasm import HiQASM
    ...
    def to_qcis(self, file_name: Optional[str] = None, parametric: bool = True) -> str:
        """
        Convert a quafu circuit to QCIS format string and optionally save to a file.
        ...
        """
        # pylint: disable=import-outside-toplevel
        from quafu.io.qasm import QCIS
    ...
    def sx(self, obj_qubits, ctrl_qubits=None, hermitian=False):
        ...
        Args:
            ...
            hermitian (bool): Whether use the hermitian conjugated version. Default: ``False``.
        """
        gate = quafu_gates.SX.on(obj_qubits, ctrl_qubits)
        if hermitian:
            gate = gate.hermitian()
        self.append(gate)
        return self
    ...
    def swap(self, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.SWAP.on(obj_qubits, ctrl_qubits))
        return self
    ...
    def iswap(self, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.ISWAP.on(obj_qubits, ctrl_qubits))
        return self
    ...
    def swap_alpha(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.SWAPalpha(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def rx(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.RX(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def ry(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.RY(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def rz(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.RZ(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def phase_shift(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.PhaseShift(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def global_phase(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.GlobalPhase(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def givens(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.Givens(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def u3(self, theta, phi, lamda, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.U3(theta, phi, lamda).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def rxx(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.Rxx(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def rxy(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.Rxy(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def rxz(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.Rxz(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def ryy(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.Ryy(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def ryz(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.Ryz(para).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def rzz(self, para, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.Rzz(para).on(obj_qubits, ctrl_qubits))
        return self

    @staticmethod
    def from_openqasm(openqasm_str: str):
        """
        Convert an OpenQASM string or an OpenQASM file to quafu circuit.

        Args:
            openqasm_str (str): String format of OpenQASM or an OpenQASM file name.

        Returns:
            :class:`~.core.circuit.Circuit`, The quafu circuit converted from OpenQASM.
        """
        # pylint: disable=import-outside-toplevel
        from quafu.io.qasm import OpenQASM
    ...
    def from_hiqasm(hiqasm_str: str):
        """
        Convert a HiQASM string or a HiQASM file to quafu circuit.

        Args:
            hiqasm_str (str): String format of HiQASM or a HiQASM file name.

        Returns:
            :class:`~.core.circuit.Circuit`, The quafu circuit converted from HiQASM.
        """
        # pylint: disable=import-outside-toplevel
        from quafu.io.qasm import HiQASM
    ...
    def from_qcis(qcis_str: str):
        """
        Convert an QCIS string or an QCIS file to quafu circuit.

        Args:
            qcis_str (str): String format of QCIS or an QCIS file name.

        Returns:
            :class:`~.core.circuit.Circuit`, The quafu circuit converted from QCIS.
        """
        # pylint: disable=import-outside-toplevel
        from quafu.io.qasm import QCIS
    ...
    def fsim(self, theta, phi, obj_qubits, ctrl_qubits=None):
        ...
        """
        self.append(quafu_gates.FSim(theta, phi).on(obj_qubits, ctrl_qubits))
        return self
    ...
    def measure(self, key, obj_qubit=None, reset_to=None):
        ...
        if obj_qubit is None:
            self.append(quafu_gates.Measure(reset_to=reset_to).on(key))
        else:
            self.append(quafu_gates.Measure(key, reset_to=reset_to).on(obj_qubit))
        return self
    ...
    def measure_all(self, suffix=None, up_to: int = -1):
        ...
        for i in range(max(self.n_qubits, up_to)):
            ...
            self += quafu_gates.Measure(string).on(i)
        return self
    ...
    def barrier(self, show=True):
        ...
        """
        _check_input_type('show', bool, show)
        self.append(quafu_gates.BarrierGate(show))
        return self
    ...
    def un(self, gate, maps_obj, maps_ctrl=None):
        ...
        """
        from quafu import UN  # pylint: disable=import-outside-toplevel

        self += UN(gate, maps_obj, maps_ctrl)
        return self

    def get_qs(self, backend='quafuvector', pr=None, ket=False, seed=None, dtype=None):
        """
        Get the final quantum state of this circuit.

        Args:
            backend (str): Which backend you want to use. Default: ``'quafuvector'``.
            ...
            dtype (quafu.dtype): The data type of simulator.
        """
        from quafu import (  # pylint: disable=import-outside-toplevel,cyclic-import
            Simulator,
        )
    ...
    def svg(self, style=None, width=None, scale=None):
        ...
        from quafu.io.display.circuit_svg_drawer import SVGCircuit
    ...
    def remove_noise(self):
        ...
        circ = Circuit()
        for gate in self:
            if not isinstance(gate, quafu_gates.NoiseGate):
                circ += gate
        return circ

    def with_noise(self, noise_gate=quafu_gates.AmplitudeDampingChannel(0.001), also_ctrl=False):
        """
        ...
        for gate in self:
            circ += gate
            if not isinstance(gate, (quafu_gates.Measure, quafu_gates.NoiseGate)):
