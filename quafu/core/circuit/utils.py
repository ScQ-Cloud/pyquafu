...
from quafu.core.gates.basic import ParameterGate
...
def decompose_single_term_time_evolution(term, para):  # pylint: disable=too-many-branches
    ...
    Examples:
        ...
        >>> from quafu.core.circuit import decompose_single_term_time_evolution
    ...
    from quafu.utils.type_value_check import _num_type
...
def pauli_word_to_circuits(qubitops):
    ...
    Examples:
        >>> from quafu.core import X
        >>> from quafu.core.operators import QubitOperator
        >>> from quafu.core.circuit import pauli_word_to_circuits
    ...
    from quafu import operators as ops
    from quafu.core import gates
...
def controlled(circuit_fn):
    ...
    Examples:
        ...
        >>> from quafu.core.circuit import controlled
        >>> u1 = qft([0, 1])
        >>> u2 = controlled(u1)
        >>> u3 = controlled(qft)
        >>> u3 = u3(2, [0, 1])
...
def dagger(circuit_fn):
    ...
    Examples:
        >>> from quafu.algorithm.library import qft
        >>> from quafu.core.circuit import dagger
        >>> u1 = qft([0, 1])
        >>> u2 = dagger(u1)
        >>> u3 = dagger(qft)
        >>> u3 = u3([0, 1])
...
def add_prefix(circuit_fn, prefix: str):
    ...
    Examples:
        >>> from quafu.algorithm.library import qft
        >>> from quafu.core.circuit import add_prefix
        >>> from quafu import RX, H, Circuit
...
def add_suffix(circuit_fn, suffix: str):
    ...
    Examples:
        >>> from quafu.algorithm.library import qft
        >>> from quafu.core.circuit import add_suffix
        >>> from quafu import RX, H, Circuit
        >>> u = lambda qubit: Circuit([H.on(0), RX('a').on(qubit)])
        >>> u1 = u(0)
        >>> u2 = add_suffix(u1, '1')
...
def shift(circ, inc):
    ...
    Examples:
        >>> from quafu.core.circuit import shift, Circuit
        >>> circ = Circuit().x(1, 0)
        >>> circ
        q0: ────■─────
                ┃
...
def change_param_name(circuit_fn, name_map):
    ...
    Examples:
        >>> from quafu.algorithm.library import qft
        >>> from quafu.core.circuit import change_param_name, Circuit
...
def as_encoder(circuit_fn):
    ...
    Examples:
        >>> from quafu.core.circuit import as_encoder, Circuit
...
def as_ansatz(circuit_fn):
    ...
    Examples:
        >>> from quafu.core.circuit import as_ansatz, Circuit
        >>> from quafu.core.gates import RX
        >>> @as_ansatz
        ... def create_circuit():
        ...     circ = Circuit()
