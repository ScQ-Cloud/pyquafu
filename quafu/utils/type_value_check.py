...
def _check_hamiltonian_qubits_number(hamiltonian, sim_qubits):
    ...
    from quafu.core.operators.hamiltonian import HowTo  # noqa: E402
...
def _check_quafu_type(dtype):
    # pylint: disable=import-outside-toplevel,cyclic-import
    from quafu.dtype.dtype import quafu_number_type

    if dtype not in quafu_number_type:
        raise ValueError(f"dtype only support {quafu_number_type}, but get {dtype}")
