...
if TYPE_CHECKING:
    from quafu.core import BasicGate, Circuit
...
def random_clifford_circuit(n_qubits: int, gate_num: int, seed: int = None) -> Circuit:
    ...
    Examples:
        >>> import numpy as np
        >>> from quafu.utils import random_clifford_circuit
...
def random_insert_gates(
    ...
    Examples:
        >>> from quafu.core.gates import X, Z, BitFlipChannel, PhaseFlipChannel
        >>> from quafu.core.circuit import Circuit
        >>> from quafu.utils import random_insert_gates
...
def random_hamiltonian(n_qubits: int, n_terms: int, seed: int = None, dtype=None):
    ...
    Args:
        ...
        dtype (quafu.dtype): data type of hamiltonian. Default: ``None``.
...
def is_two_number_close(a, b, atol=None):  # pylint: disable=invalid-name
    ...
    Examples:
        ...
        True
    """
    from quafu.config.config import (  # pylint: disable=import-outside-toplevel
        context,
    )
