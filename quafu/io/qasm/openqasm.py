...
def cnot_related(gate):
    """Convert quafu gate to qasm."""
    from quafu.core import gates as G  # noqa: N812
...
def generate_custom_gate(gate_def, custom_gate, gate_map_openqasm_quafu):
    ...
    def gate(prs, qids):
        ...
        for cmd in body:
            ...
            if sub_gate_name not in gate_map_openqasm_quafu:
                ...
                circ += gate_map_openqasm_quafu[sub_gate_name](real_prs, real_qid)
        return circ

    custom_gate[gate_name] = gate


def quafu_to_qasm_v2(circ, gate_map_quafu_openqasm, version: str = '2.0'):
    """Convert quafu circuit to openqasm."""
    from quafu.algorithm.compiler import BasicDecompose, compile_circuit
    from quafu.core import gates as G  # noqa: N812
    from quafu.core.circuit import Circuit
    ...
    for gate in circ:
        ...
        if isinstance(gate, G.Measure):
            ...
            continue
        if gate.__class__ in gate_map_quafu_openqasm:
            cmds.append(gate_map_quafu_openqasm[gate.__class__](gate))
            continue
        compiled_circ = compile_circuit(BasicDecompose(), Circuit([gate]))
        for sub_gate in compiled_circ:
            if sub_gate.__class__ in gate_map_quafu_openqasm:
                cmds.append(gate_map_quafu_openqasm[sub_gate.__class__](sub_gate))
            else:
...
def qasm_to_quafu_v2(qasm: str, gate_map_openqasm_quafu):
    """Convert openqasm to quafu circuit."""
    from quafu.core.circuit import Circuit
    ...
    for i in cmds:
        ...
        if i.startswith('gate '):
            generate_custom_gate(i, custom_gate, gate_map_openqasm_quafu)
        else:
            cmd = i.strip()
            gate_name, params, qids = parse_gate_cmd(cmd, qregs, cregs)
            if gate_name == 'barrier':
                circ += gate_map_openqasm_quafu['barrier'](params, [q[0] for q in qids])
                continue
            for qid in qids:
                if gate_name in custom_gate:
                    circ += custom_gate[gate_name](params, qid)
                elif gate_name in gate_map_openqasm_quafu:
                    quafu_gate = gate_map_openqasm_quafu[gate_name](params, qid)
                    if gate_name == 'measure':
                        quafu_gate.key = f"q{qid[0]}_{measures.get(qid[0], 0)}"
...
class OpenQASM:
    ...
    Examples:
        >>> import numpy as np
        >>> from quafu.core import Circuit
    ...
    def to_string(self, circuit, version="2.0"):  # pylint: disable=R0912,R0914,R0915
        ...
        """
        return quafu_to_qasm_v2(circuit, self.gate_map_quafu_openqasm, version)
    ...
    def to_file(self, file_name, circuit, version="2.0"):
        ...
        Raises:
            ...
            TypeError: if `version` is not a str.
        """
        from quafu.core import (  # pylint: disable=import-outside-toplevel,cyclic-import
            Circuit,
        )
    ...
    def from_file(self, file_name):
        ...
        with fdopen(file_name, 'r') as fd:
            cmds = fd.readlines()
        return qasm_to_quafu_v2('\n'.join(cmds), self.gate_map_openqasm_quafu)
    ...
    def from_string(self, string):
        ...
        return qasm_to_quafu_v2(string, self.gate_map_openqasm_quafu)
