...
class DAGCircuit(MultiDiGraph):
    ...
    def node_qubits_inedges(self, node: InstructionNode):
        ...
        inedges = [edge for edge in self.in_edges(node, data=True, keys=True)]  # we can get u,v,k,d
        qubits_labels = [int(edge[2]['label'][1:]) for edge in self.in_edges(node, data=True)]
    ...
    def node_qubits_outedges(self, node: InstructionNode):
        ...
        if node in [float('inf')]:
            raise ValueError('float("inf") has no successors')
        outedges = [edge for edge in self.out_edges(node, data=True, keys=True)]  # we can get u,v,k,d
        qubits_labels = [int(edge[2]['label'][1:]) for edge in self.out_edges(node, data=True)]
    ...
    def merge_dag(self, other_dag):
        ...
        if len(insect_qubits) != 0:
            for insect_qubit in insect_qubits:
                ...
                self.add_edge(end_edges_labels_1[insect_qubit][0], start_edges_labels_2[insect_qubit][1],
    ...
    def substitute_node_with_dag(self, gate: InstructionNode, input_dag):
        ...
        for qubit in input_dag_qubits:
            if predecessors_dict[qubit] == -1:
                self.add_edge(predecessors_dict[qubit], input_dag_startnodes[qubit], label=f'q{qubit}', color='green')
            else:
                self.add_edge(predecessors_dict[qubit], input_dag_startnodes[qubit], label=f'q{qubit}')
            if successors_dict[qubit] == float('inf'):
                self.add_edge(input_dag_endnodes[qubit], successors_dict[qubit], label=f'q{qubit}', color='red')
            else:
