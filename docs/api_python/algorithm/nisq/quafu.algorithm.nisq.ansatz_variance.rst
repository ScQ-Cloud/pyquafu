...
.. py:function:: quafu.algorithm.nisq.ansatz_variance(ansatz: Circuit, ham: Hamiltonian, focus: str, var_range: typing.Tuple[float, float] = (0, np.pi * 2), other_var: np.array = None, atol: float = 0.1, init_batch: int = 20, sim: typing.Union[Simulator, str] = 'quafuvector')
    ...
    参数：
        ...
        - **sim** (Union[:class:`~.simulator.Simulator`, str]) - 用哪种模拟器来完成任务。默认值： ``quafuvector``。
