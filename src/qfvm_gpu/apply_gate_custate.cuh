...
void apply_gate_custate(cuDoubleComplex* psi_d, QuantumOperator& op, int n) {
    ...
    custatevecApplyMatrixGetWorkspaceSize(handle, CUDA_C_64F, n, mat, CUDA_C_64F, CUSTATEVEC_MATRIX_LAYOUT_ROW, adjoint,
                                          nTargets, nControls, CUSTATEVEC_COMPUTE_64F, &extraWorkspaceSizeInBytes);

    // allocate external workspace if necessary
    if (extraWorkspaceSizeInBytes > 0)
        cudaMalloc(&extraWorkspace, extraWorkspaceSizeInBytes);

    custatevecApplyMatrix(handle, psi_d, CUDA_C_64F, n, mat, CUDA_C_64F, CUSTATEVEC_MATRIX_LAYOUT_ROW, adjoint,
                          targets.data(), nTargets, controls.data(), nullptr, nControls, CUSTATEVEC_COMPUTE_64F,
