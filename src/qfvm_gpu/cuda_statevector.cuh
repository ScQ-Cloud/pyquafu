...
class CudaStateVector {
 ...
 public:
    // construct function
    CudaStateVector() {
        checkCudaErrors(cudaMalloc(&data_, 0));
    }
...
CudaStateVector::CudaStateVector(StateVector<double>& sv) : num_(sv.num()), size_(sv.size()) {
...
CudaStateVector::CudaStateVector(CudaStateVector const& other) : num_(other.num()), size_(other.size()) {
    checkCudaErrors(cudaMalloc(&data_, size_ * sizeof(cuDoubleComplex)));
    checkCudaErrors(cudaMemcpy(data_, other.data(), size_ * sizeof(cuDoubleComplex), cudaMemcpyDeviceToDevice));
}
