...
__global__ void apply_2to4_targe_gate_kernel(cuDoubleComplex* psi_d, uint ctrlnum, int psize) {
    ...
    for (int y = 0; y < matlen; ++y) {
        ...
        for (int x = 0; x < matlen; ++x) {
            psi_d_buffer[y] = cuCadd(psi_d_buffer[y],
                                     cuCmul(psi_d[i | mat_mask_d_const[x]], mat_d_const[y * matlen + x]));
        }
    }
    for (int y = 0; y < matlen; ++y)
...
void apply_5_targe_gate_gpu_const(cuDoubleComplex* psi_d, QuantumOperator& op, size_t size) {
    ...
    apply_5_targe_gate_kernel_const<<<rsize, blockdim, thread_num * sizeof(cuDoubleComplex)>>>(psi_d, op.control_num(),
                                                                                               psize, size);
}

// For target number 6-10
__global__ void apply_multi_targe_gate_kernel_shared(cuDoubleComplex* psi_d, uint ctrlnum, cuDoubleComplex* mat_d,
                                                     uint* mat_mask_d, int psize, size_t size) {
    uint rsize = size >> psize;
    uint targnum = psize - ctrlnum;
    uint matlen = (1 << targnum);
    uint block_length = blockDim.x;
...
void apply_multi_targe_gate_gpu_shared(cuDoubleComplex* psi_d, QuantumOperator& op, cuDoubleComplex* mat_d,
                                       uint* mat_mask_d, size_t size) {
    uint targe_num = op.targe_num();
    uint matlen = 1 << targe_num;
    auto pos = op.positions();
    uint psize = pos.size();
...
__global__ void apply_multi_targe_gate_kernel_global(cuDoubleComplex* psi_d, cuDoubleComplex* psi_d_copy, uint ctrlnum,
...
void apply_multi_targe_gate_gpu_global(cuDoubleComplex* psi_d, cuDoubleComplex* psi_d_copy, QuantumOperator& op,
                                       cuDoubleComplex* mat_d, uint* mat_mask_d, size_t size) {
    uint targe_num = op.targe_num();
    uint matlen = 1 << targe_num;
    auto pos = op.positions();
    uint psize = pos.size();
    ...
    apply_multi_targe_gate_kernel_global<<<rsize, blockdim>>>(psi_d, psi_d_copy, op.control_num(), mat_d, mat_mask_d,
                                                              psize, size);
