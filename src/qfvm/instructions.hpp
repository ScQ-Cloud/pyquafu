...
public:
  ...
  Instruction(string const& name, vector<pos_t> const& positions)
      : name_(name), positions_(positions) {}
...
public:
  // Constructor
  QuantumOperator(){};
  QuantumOperator(string const name, vector<double> paras,
                  vector<pos_t> const& control_qubits,
                  vector<pos_t> const& targe_qubits,
                  RowMatrixXcd const& mat = RowMatrixXcd(0, 0),
  ...
  QuantumOperator(string const name, vector<pos_t> const& positions,
                  RowMatrixXcd const& mat = RowMatrixXcd(0, 0),
                  RowMatrixXcd const& full_mat = RowMatrixXcd(0, 0));
...
QuantumOperator::QuantumOperator(string const name,
                                 vector<pos_t> const& positions,
                                 RowMatrixXcd const& mat,
                                 RowMatrixXcd const& full_mat)
    : Instruction(name, positions), paras_(vector<double>(0)), control_num_(-1),
      targe_num_(0), diag_(false), real_(false), targ_mat_(mat),
      full_mat_(full_mat) {}

QuantumOperator::QuantumOperator(string const name, vector<double> paras,
                                 vector<pos_t> const& positions,
                                 uint control_num, RowMatrixXcd const& mat,
                                 RowMatrixXcd const& full_mat, bool diag,
                                 bool real)
    : Instruction(name, positions), paras_(paras), control_num_(control_num),
      targe_num_(positions.size() - control_num), diag_(diag), real_(real),
      targ_mat_(mat), full_mat_(full_mat) {}
...
QuantumOperator::QuantumOperator(string const name, vector<double> paras,
                                 ...
                                 RowMatrixXcd const& full_mat, bool diag,
                                 bool real)
    : paras_(paras), diag_(diag), real_(real), targ_mat_(mat),
      full_mat_(full_mat) {
  Instruction::name_ = name;
  Instruction::positions_ = control_qubits;
  Instruction::positions_.insert(Instruction::positions_.end(),
                                 targe_qubits.begin(), targe_qubits.end());
...
public:
  Cif(vector<pos_t>& cbits, uint condition,
      vector<std::unique_ptr<Instruction>>& instructions)
      : cbits_(cbits), condition_(condition),
  ...
  if (!(name == "barrier" || name == "delay" || name == "id" ||
    ...
    if (OPMAP.count(name) == 0) {
      if (py::hasattr(obj, "_targ_matrix")) {
        targ_mat = obj.attr("_get_targ_matrix")("reverse_order"_a = true)
                       .cast<RowMatrixXcd>();
      } else { // Single gate
    ...
    if (get_full_mat) {
      full_mat = obj.attr("_get_raw_matrix")("reverse_order"_a = reverse)
                     .cast<RowMatrixXcd>();
    } else {
      full_mat = RowMatrixXcd(0, 0);
    }
    return std::make_unique<QuantumOperator>(name, paras, positions,
                                             control_num, std::move(targ_mat),
                                             std::move(full_mat));
