...
class Instruction {
 ...
 public:
    ...
    virtual bool has_control() const {
        return false;
    }
    ...
    virtual RowMatrixXcd targ_mat() const {
        return RowMatrixXcd(0, 0);
    }
    virtual RowMatrixXcd full_mat() const {
        return RowMatrixXcd(0, 0);
    }
    virtual uint control_num() const {
        return 0;
    }
    virtual uint targe_num() const {
        return 0;
    }

    virtual vector<pos_t> qbits() const {
        return {};
    }
    virtual vector<pos_t> cbits() const {
        return {};
    }

    virtual uint condition() const {
        return 0;
    }
    virtual vector<std::unique_ptr<Instruction>>& instructions() {
        vector<std::unique_ptr<Instruction>> empty = {};
        return empty;
    }
};
...
class QuantumOperator : public Instruction {
 ...
 public:
    // Constructor
    QuantumOperator(){};
    QuantumOperator(string const name, vector<double> paras, vector<pos_t> const& control_qubits,
                    vector<pos_t> const& targe_qubits, RowMatrixXcd const& mat = RowMatrixXcd(0, 0),
                    RowMatrixXcd const& full_mat = RowMatrixXcd(0, 0), bool diag = false, bool real = false);

    QuantumOperator(string const name, vector<double> paras, vector<pos_t> const& positions, uint control_num,
                    RowMatrixXcd const& mat = RowMatrixXcd(0, 0), RowMatrixXcd const& full_mat = RowMatrixXcd(0, 0),
                    bool diag = false, bool real = false);

    QuantumOperator(string const name, vector<pos_t> const& positions, RowMatrixXcd const& mat = RowMatrixXcd(0, 0),
                    RowMatrixXcd const& full_mat = RowMatrixXcd(0, 0));
    // virtual ~QuantumOperator();

    // data accessor
    virtual vector<double> paras() const override {
        return paras_;
    }
    virtual bool has_control() const override {
        return control_num_ == 0 ? false : true;
    }
    virtual bool is_real() const override {
        return real_;
    }
    virtual bool is_diag() const override {
        return diag_;
    }
    virtual RowMatrixXcd targ_mat() const override {
        return targ_mat_;
    }
    virtual RowMatrixXcd full_mat() const override {
        return full_mat_;
    }
    virtual uint control_num() const override {
        return control_num_;
    }
    virtual uint targe_num() const override {
        return targe_num_;
    }
};

QuantumOperator::QuantumOperator(string const name, vector<pos_t> const& positions, RowMatrixXcd const& mat,
                                 RowMatrixXcd const& full_mat)
...
QuantumOperator::QuantumOperator(string const name, vector<double> paras, vector<pos_t> const& positions,
                                 uint control_num, RowMatrixXcd const& mat, RowMatrixXcd const& full_mat, bool diag,
                                 bool real)
    : Instruction(name, positions)
    , paras_(paras)
...
QuantumOperator::QuantumOperator(string const name, vector<double> paras, vector<pos_t> const& control_qubits,
                                 vector<pos_t> const& targe_qubits, RowMatrixXcd const& mat,
                                 RowMatrixXcd const& full_mat, bool diag, bool real)
    : paras_(paras), diag_(diag), real_(real), targ_mat_(mat), full_mat_(full_mat) {
...
class Measures : public Instruction {
 ...
 public:
    Measures(vector<pos_t>& qbits, vector<pos_t>& cbits) : qbits_(qbits), cbits_(cbits) {
    Instruction:
        name_ = "measure";
    }
    virtual vector<pos_t> qbits() const override {
        return qbits_;
    }
    virtual vector<pos_t> cbits() const override {
        return cbits_;
    }
};
...
class Reset : public Instruction {
 ...
 public:
    Reset(vector<pos_t> const& qbits) : qbits_(qbits) {
    Instruction:
        name_ = "reset";
    }
    virtual vector<pos_t> qbits() const override {
...
class Cif : public Instruction {
 ...
 public:
    Cif(vector<pos_t>& cbits, uint condition, vector<std::unique_ptr<Instruction>>& instructions)
        : cbits_(cbits), condition_(condition), instructions_(std::move(instructions)) {
    Instruction:
        name_ = "cif";
    }
    virtual uint condition() const override {
        return condition_;
    }
    virtual vector<std::unique_ptr<Instruction>>& instructions() override {
...
std::unique_ptr<Instruction> from_pyops(py::object const& obj, bool get_full_mat = false, bool reverse = true) {
    ...
    if (!(name == "barrier" || name == "delay" || name == "id" || name == "measure" || name == "reset"
          || name == "cif")) {
        // QuantumGate
        ...
        return std::make_unique<QuantumOperator>(name, paras, positions, control_num, std::move(targ_mat),
                                                 std::move(full_mat));
