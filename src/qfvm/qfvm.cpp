...
py::array_t<T> to_numpy(const std::tuple<T*, size_t>& src) {
    ...
    auto capsule = py::capsule(src_ptr, [](void* p) { delete[] reinterpret_cast<T*>(p); });
...
std::pair<std::map<uint, uint>, py::array_t<complex<double>>> simulate_circuit(
    py::object const& pycircuit, py::array_t<complex<double>>& np_inputstate, const int& shots) {
...
std::map<uint, uint> simulate_circuit_clifford(py::object const& pycircuit, const int& shots) {
    auto circuit = Circuit(pycircuit);
    ...
    for (uint i = 0; i < actual_shots; i++) {
        ...
        if (!circuit.final_measure()) {
            ...
            for (auto& measure : measures) {
                cs.do_circuit_instruction({"measure", std::vector<size_t>{measure.first},
...
PYBIND11_MODULE(qfvm, m) {
    ...
    m.def("simulate_circuit", &simulate_circuit, "Simulate with circuit", py::arg("circuit"),
          py::arg("inputstate") = py::array_t<complex<double>>(0), py::arg("shots"));
    m.def("simulate_circuit_clifford", &simulate_circuit_clifford, "Simulate with circuit using clifford",
          py::arg("circuit"), py::arg("shots"));

    m.def("expect_statevec", &expect_statevec, "Calculate paulis expectation", py::arg("inputstate"),
          py::arg("paulis"));

    m.def("applyop_statevec", &applyop_statevec, "Apply single operator to state", py::arg("operation"),
          py::arg("inputstate"));

    m.def("sampling_statevec", &sampling_statevec, "sampling state", py::arg("measures"), py::arg("inputstate"),
          py::arg("shots"));

#ifdef _USE_GPU
    m.def("simulate_circuit_gpu", &simulate_circuit_gpu, "Simulate with circuit", py::arg("circuit"),
          py::arg("inputstate") = py::array_t<complex<double>>(0));
#endif

#ifdef _USE_CUQUANTUM
    m.def("simulate_circuit_custate", &simulate_circuit_custate, "Simulate with circuit", py::arg("circuit"),
          py::arg("inputstate") = py::array_t<complex<double>>(0));
#endif
}
