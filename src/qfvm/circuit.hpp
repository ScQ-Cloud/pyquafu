...
public:
  ...
  Circuit(py::object const& pycircuit, bool get_full_mat = false,
          bool reverse = true);
...
Circuit::Circuit(py::object const& pycircuit, bool get_full_mat, bool reverse)
  ...
  qubit_num_ =
      pycircuit.attr("num")
          .cast<uint>(); // To consist with other simulators  (e.g. qiskit)
  ...
  for (auto pyop_h : pyops) {
    ...
    if (py::hasattr(pyop, "circuit")) { // handle oracle
      ...
      std::unique_ptr<Instruction> ins =
          from_pyops(pyop, get_full_mat, reverse);
      //   check_operator(*(ins));
      if (*ins) {
        if (ins->name() == "measure") {
          measured = true;
          ...
          for (uint i = 0; i < ins->qbits().size(); i++) {
            measure_vec_.push_back(
                std::make_pair(ins->qbits()[i], ins->cbits()[i]));
          }
        } else {
