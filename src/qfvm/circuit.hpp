...
class Circuit {
 ...
 public:
    ...
    uint qubit_num() const {
        return qubit_num_;
    }
    uint cbit_num() const {
        return cbit_num_;
    }
    uint max_targe_num() const {
        return max_targe_num_;
    }
    bool final_measure() const {
        return final_measure_;
    }
    vector<QuantumOperator> gates();
    vector<std::pair<uint, uint>> measure_vec() {
        return measure_vec_;
    }
...
Circuit::Circuit(vector<std::unique_ptr<Instruction>>& ops) : instructions_(std::move(ops)), max_targe_num_(0) {
    qubit_num_ = 0;
