...
void apply_op_general(StateVector<data_t>& state, Instruction const& op) {
    if (op.targe_num() == 1) {
...
void apply_op(Instruction& op, StateVector<data_t>& state) {
    bool matched = false;
...
void apply_measure(circuit_simulator<word_size>& cs, const vector<pos_t>& qbits, const vector<pos_t>& cbits) {
    for (size_t i = 0; i < qbits.size(); i++) {
        cs.do_circuit_instruction(
            {"measure", std::vector<size_t>{qbits[i]}, std::vector<double>{static_cast<double>(cbits[i])}});
    }
}
...
void apply_op(Instruction& op, circuit_simulator<word_size>& cs) {
    ...
    switch (OPMAP.at(op.name())) {
        ...
        default:
            ...
            cs.do_circuit_instruction({op.name(), std::vector<size_t>(qubits.begin(), qubits.end())});
    }
}
