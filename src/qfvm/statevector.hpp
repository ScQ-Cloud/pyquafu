...
template <class real_t>
StateVector<real_t>::StateVector(complex<real_t>* data, size_t data_size) : data_(data), size_(data_size) {
    num_ = static_cast<int>(std::log2(size_));
}
...
void StateVector<real_t>::apply_rx(pos_t pos, real_t theta) {
    complex<double> mat[4] = {std::cos(theta / 2), -imag_I * std::sin(theta / 2), -imag_I * std::sin(theta / 2),
                              std::cos(theta / 2)};
...
void StateVector<real_t>::apply_one_targe_gate_general(vector<pos_t> const& posv, complex<double>* mat) {
    ...
    if (ctrl_num == 0) {
        ...
        getind_func = [&](size_t j) -> size_t { return (j & (offset - 1)) | (j >> targe << targe << 1); };
...
template <class real_t>
void StateVector<real_t>::apply_multi_targe_gate_general(vector<pos_t> const& posv, uint control_num,
...
#pragma omp parallel for
            ...
            if (diag[0] == 0.0) {
                // [[1, 0], [0, 0]]
                auto func = [&](const indexes_t& inds) -> void { data_[inds[1]] = 0.0; };
#pragma omp parallel for
                for (int k = 0; k < (size_ >> 1); k += 1) {
                    const auto inds = indexes(qbit0, qbit0, k);
                    func(inds);
...
#pragma omp parallel for
            ...
            if (diag[0] == 0.0) {
                // [[0, 0], [0, 1]]
                auto func = [&](const indexes_t& inds) -> void { data_[inds[0]] = 0.0; };
#pragma omp parallel for
                for (int k = 0; k < (size_ >> 1); k += 1) {
                    const auto inds = indexes(qbit0, qbit0, k);
                    func(inds);
...
template <class real_t>
void StateVector<real_t>::update(vector<pos_t> const& qbits, const uint final_state, const uint meas_state,
...
// sample results
template <class real_t>
std::unordered_map<std::string, int> StateVector<real_t>::measure_samples(vector<std::pair<uint, uint>> meas,
                                                                          int shots) {
