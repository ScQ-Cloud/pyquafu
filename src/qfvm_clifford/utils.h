...
constexpr size_t bits_to_bits_padded(size_t bits) {
    return (bits + (word_size - 1)) & ~(word_size - 1);
}

template <size_t word_size>
constexpr size_t bits_to_word_padded(size_t bits) {
...
inline uint8_t count_uint64_bits(uint64_t value) {
    ...
    return (((value + (value >> 4)) & 0xF0F0F0F0F0F0F0Full) * 0x101010101010101ull) >> 56;
}
