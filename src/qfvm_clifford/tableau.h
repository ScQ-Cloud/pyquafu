...
enum gate_type { SINGLE_QUBIT_GATE, TWO_QUBIT_GATE, COLLAPSING_GATE, ERROR_QUBIT_GATE };
...
// collasping gate
template <size_t word_size>
using func_type = std::variant<std::function<result(tableau<word_size>& t, const size_t qubit)>,
                               std::function<result(tableau<word_size>& t, const size_t qubit1, const size_t qubit2)>,
                               std::function<result(tableau<word_size>& t, std::mt19937_64& rng, const size_t qubit)>>;
...
// is the gate name
template <size_t word_size>
std::unordered_map<std::string, std::pair<gate_type, func_type<word_size>>> gate_map = {
...
struct _tableau {
    ...
    pauli_string_slice<word_size> operator[](size_t qubit) {
        ...
        return pauli_string_slice<word_size>(num_qubits, signs[qubit], xs_t[qubit].slice(0, num_words),
...
struct tableau {
    ...
    // constructor
    explicit tableau(size_t num_qubits) : num_qubits(num_qubits), distabilizer(num_qubits), stabilizer(num_qubits) {
...
#undef STRUCT_FUNCTION_REGISTRATION
    ...
    void reset() {
        *this = tableau<word_size>(num_qubits);
    }
    void reset(std::mt19937_64& rng, size_t qubit) {
        r_gate(rng, qubit);
    }
    ...
    void expand(size_t new_num_qubits, double resize_pad_factor) {
        ...
        *this = tableau<word_size>((size_t) (new_num_qubits * resize_pad_factor));
        ...
        auto partial_copy = [=](packed_bit_word_slice<word_size> dst, packed_bit_word_slice<word_size> src) {
    ...
    pauli_string<word_size> eval_y_obs(size_t qubit) const {
        ...
        uint8_t log_i = pauli_string_slice<word_size>(result).inplace_right_mul(stabilizer[qubit]);
        log_i++;
        assert((log_i & 1) == 0);
        if (log_i & 2) {
            result.sign ^= true;
    ...
    size_t collapse_qubit_along_z(tableau_trans<word_size>& t_trans, size_t target_qubit, std::mt19937_64& rng) {
        size_t pivot = 0;

        // search for any generator that anti-commutes with the measurement
        // observable
        while (pivot < num_qubits && !t_trans.t.stabilizer.xs_t[pivot][target_qubit])
            pivot++;
    ...
    static tableau<word_size> random_valid_stabilizer_tableau(size_t num_qubits, std::mt19937_64& rng) {
        ...
        for (size_t row = 0; row < num_qubits; row++) {
            for (size_t col = 0; col < num_qubits; col++) {
                ...
                result.stabilizer[row].zs[col] = raw[row + num_qubits][col + num_qubits];
            }
        }
...
std::ostream& operator<<(std::ostream& out, const tableau<word_size>& t) {
    ...
    for (size_t q = 0; q < t.num_qubits; q++) {
        out << " |\n|";
        for (size_t k = 0; k < t.num_qubits; k++) {
            out << ' ' << "IXZY"[t.distabilizer[k].xs[q] | t.distabilizer[k].zs[q] << 1]
...
struct tableau_trans {
    ...
    tableau_trans<word_size>& cnot_gate(const size_t control_qubit, const size_t target_qubit) {
        for_each_trans_obs(control_qubit, target_qubit, [](auto& cx, auto& cz, auto& tx, auto& tz, auto& s) {
