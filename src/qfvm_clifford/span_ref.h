...
struct span_ref {
    ...
    span_ref(std::vector<T>& items) : ptr_start(items.data()), ptr_end(items.data() + items.size()) {
    }
    ...
    template <size_t K>
    span_ref(std::array<T, K>& items) : ptr_start(items.data()), ptr_end(items.data() + items.size()) {
    }
    ...
    bool operator==(const span_ref<typename std::remove_const<T>::type>& other) const {
        return span_ref<const T>(ptr_start, ptr_end) == span_ref<const T>(other.ptr_start, other.ptr_end);
    }
...
struct monotonic_buffer {
    ...
    // region.
    monotonic_buffer(size_t reserve) {
