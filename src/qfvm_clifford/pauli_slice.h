...
struct pauli_string_slice {
    ...
    // zs.num_bit_words
    pauli_string_slice(size_t num_qubits, bit sign, packed_bit_word_slice<word_size> xs,
                       packed_bit_word_slice<word_size> zs)
        : num_qubits(num_qubits), sign(sign), xs(xs), zs(zs) {
    }

    // assign operator
    pauli_string_slice<word_size>& operator=(const pauli_string_slice<word_size>& other) {
    ...
    pauli_string_slice<word_size>& mul_ignore_anti_commute(const pauli_string_slice<word_size>& other) {
        auto res = inplace_right_mul(other);
        // if the result phase is positive, then compute excluse-or of the signs
        sign ^= res & 2;
        return *this;
    ...
    uint8_t inplace_right_mul(const pauli_string_slice<word_size>& other) noexcept {
        bit_word<word_size> count1{};
        bit_word<word_size> count2{};

        xs.for_each_word(zs, other.xs, other.zs, [&count1, &count2](auto& x1, auto& z1, auto& x2, auto& z2) {
    ...
    bool commutes(const pauli_string_slice<word_size>& other) const noexcept {
        ...
        xs.for_each_word(zs, other.xs, other.zs,
                         [&count](auto& x1, auto& z1, auto& x2, auto& z2) { count ^= (x1 & z2) ^ (x2 & z1); });
        return (count.count() & 1) == 0;
    }
};

template <size_t word_size>
std::ostream& operator<<(std::ostream& os, const pauli_string_slice<word_size> str) {
    os << "+-"[str.sign];
