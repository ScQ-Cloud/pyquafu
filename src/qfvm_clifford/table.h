...
struct table {
    ...
    table(size_t min_bits_major, size_t min_bits_minor)
        ...
        , data(bits_to_bits_padded<word_size>(min_bits_major) * bits_to_bits_padded<word_size>(min_bits_minor)) {
    }
    ...
    // index operator, major index should be the number of rows
    inline const packed_bit_word_slice<word_size> operator[](size_t major_index) const {
        return data.slice(major_index * num_bit_words_minor, num_bit_words_minor);
    }

    // equality operator
    bool operator==(const table<word_size>& other) const {
        return num_bit_words_major == other.num_bit_words_major && num_bit_words_minor == other.num_bit_words_minor
               && data == other.data;
    }
    ...
    // bit_word
    inline size_t get_index_bit_word(const size_t major_index, const size_t minor_index,
                                     const size_t major_index_sub) const {
        auto index = (major_index << bit_word<word_size>::BIT_POW) + major_index_sub;
        return index * num_bit_words_minor + minor_index;
    }

    // transpose the table
    table<word_size> transpose() const {
        table<word_size> result(num_bit_words_minor * word_size, num_bit_words_major * word_size);

        for (size_t major_word = 0; major_word < num_bit_words_major; major_word++) {
            for (size_t minor_word = 0; minor_word < num_bit_words_minor; minor_word++) {
                for (size_t major_word_sub = 0; major_word_sub < word_size; major_word_sub++) {
                    size_t src_index = get_index_bit_word(major_word, minor_word, major_word_sub);
                    size_t dst_index = result.get_index_bit_word(minor_word, major_word, major_word_sub);
        ...
        // word_size)
        for (size_t major_word = 0; major_word < result.num_bit_words_major; major_word++) {
            for (size_t minor_word = 0; minor_word < result.num_bit_words_minor; minor_word++) {
                size_t block_start = result.get_index_bit_word(major_word, minor_word, 0);
                bit_word<word_size>::inplace_transpose_square(result.data.bw + block_start, result.num_bit_words_minor);
            }
        }
    ...
    table<word_size>& inplace_transpose() {
        ...
        for (size_t major_word = 0; major_word < num_bit_words_major; major_word++) {
            for (size_t minor_word = 0; minor_word < num_bit_words_minor; minor_word++) {
                ...
                bit_word<word_size>::inplace_transpose_square(data.bw + block_start, num_bit_words_minor);
            }
        }

        // transpose the table
        for (size_t major_word = 0; major_word < num_bit_words_major; major_word++)
            for (size_t minor_word = major_word + 1; minor_word < num_bit_words_minor; minor_word++)
                for (size_t major_word_sub = 0; major_word_sub < word_size; major_word_sub++)
                    std::swap(data.bw[get_index_bit_word(major_word, minor_word, major_word_sub)],
                              data.bw[get_index_bit_word(minor_word, major_word, major_word_sub)]);

        return *this;
    }

    // square matrix multiplication (assuming row indexing)
    table<word_size> square_matrix_mul(const table<word_size>& right, size_t n) const {
        ...
        for (std::size_t i = 0; i < n; i++) {
            for (std::size_t j = 0; j < n; j++) {
                ...
                (*this)[i].for_each_word(tmp[j], [&](auto& a, auto& b) { accumulater ^= a & b; });
    ...
    static table<word_size> concatenate_four(size_t n, const table<word_size>& upper_left,
                                             const table<word_size>& upper_right, const table<word_size>& lower_left,
    ...
    static table<word_size> random(size_t random_bits_major, size_t random_bits_minor, std::mt19937_64& rng) {
    ...
    static inline std::pair<std::vector<bool>, std::vector<size_t>> sample_quantum_mallows(size_t n,
                                                                                           std::mt19937_64& rng) {
    ...
    static table<word_size> random_valid_stabilizer_table(size_t n, std::mt19937_64& rng) {
        ...
        auto fused = table<word_size>::concatenate_four(n, lower, table<word_size>(n, n), prod, inv);
        auto fused_m = table<word_size>::concatenate_four(n, lower_m, table<word_size>(n, n), prod_m, inv_m);
