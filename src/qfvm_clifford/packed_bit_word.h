...
struct packed_bit_word {
    ...
    explicit packed_bit_word(size_t num_bits)
        : num_bit_words(bits_to_word_padded<word_size>(num_bits)), u64(malloc_aligned_padded(num_bits)) {
    }
    ...
    packed_bit_word(const packed_bit_word& other)
        : num_bit_words(other.num_bit_words), u64(malloc_aligned_padded(other.num_bit_words * word_size)) {
    ...
    packed_bit_word(packed_bit_word&& other) noexcept : num_bit_words(other.num_bit_words), u64(other.u64) {
        other.u64 = nullptr;
    ...
    packed_bit_word<word_size>& operator=(const packed_bit_word_slice<word_size>& other) {
    ...
    bool operator==(const packed_bit_word<word_size>& other) const {
        return num_bit_words == other.num_bit_words && memcmp(bw, other.bw, word_size * num_bit_words / 8) == 0;
    }
    ...
    // assignment
    packed_bit_word<word_size>& operator^=(const packed_bit_word<word_size>& other) {
        packed_bit_word_slice<word_size>(*this) ^= packed_bit_word_slice<word_size>(other);
        return *this;
    }
    ...
    // compare operation
    bool operator<(const packed_bit_word<word_size>& other) const {
        return packed_bit_word_slice<word_size>(*this) < packed_bit_word_slice<word_size>(other);
    }
    ...
    void swap(packed_bit_word<word_size> other) {
        packed_bit_word_slice<word_size>(*this).swap(packed_bit_word_slice<word_size>(other));
    }

    // slice operator, offset and num_bit_words should be the number of bit words
    inline packed_bit_word_slice<word_size> slice(size_t offset, size_t num_bit_words) {
        return packed_bit_word_slice<word_size>(bw + offset, num_bit_words);
    }

    // slice operator, offset and num_bit_words should be the number of bit words
    inline const packed_bit_word_slice<word_size> slice(size_t offset, size_t num_bit_words) const {
        return packed_bit_word_slice<word_size>(bw + offset, num_bit_words);
    }
    ...
    static packed_bit_word<word_size> random(size_t num_bits, std::mt19937_64& rng) {
    ...
    void truncated_overwrite_from(packed_bit_word_slice<word_size>& other, size_t num_bits) {
    ...
    size_t count() {
        return packed_bit_word_slice<word_size>(*this).count();
    }
...
// bit operations
template <size_t word_size>
packed_bit_word<word_size> operator^(const packed_bit_word_slice<word_size>& left,
    ...
    packed_bit_word_slice<word_size>(result).for_each_word(left, right, [](auto& a, auto& b, auto& c) { a = b ^ c; });
    return result;
}

template <size_t word_size>
packed_bit_word<word_size> operator^(const packed_bit_word<word_size>& left, const packed_bit_word<word_size>& right) {
    return packed_bit_word_slice<word_size>(left) ^ packed_bit_word_slice<word_size>(right);
}
...
template <size_t word_size>
std::ostream& operator<<(std::ostream& os, const packed_bit_word<word_size> word) {
