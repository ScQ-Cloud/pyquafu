...
struct pauli_string {
    ...
    explicit pauli_string(std::string& str) : num_qubits(0), sign(false), xs(0), zs(0) {
        *this = std::move(from_cstr(str.c_str()));
    }

    // copy constructor
    pauli_string(const pauli_string<word_size>& other)
        : num_qubits(other.num_qubits), sign(other.sign), xs(other.xs), zs(other.zs) {
    }
    ...
    pauli_string(pauli_string<word_size>&& other) noexcept
        : num_qubits(other.num_qubits), sign(other.sign), xs(std::move(other.xs)), zs(std::move(other.zs)) {
    }
    ...
    pauli_string<word_size>& operator=(const pauli_string_slice<word_size>& other) {
        this->num_qubits = other.num_qubits, this->sign = other.sign;
        this->xs = other.xs, this->zs = other.zs;
    ...
    bool operator==(const pauli_string<word_size>& other) const {
        return num_qubits == other.num_qubits && bool(sign) == bool(other.sign) && xs == other.xs && zs == other.zs;
    }
    ...
    operator const pauli_string_slice<word_size>() const {
        return pauli_string_slice<word_size>(num_qubits, bit((void*) &sign, 0), xs, zs);
    }
    ...
    static pauli_string<word_size> random(size_t num_qubits, std::mt19937_64& rng) {
    ...
    static pauli_string<word_size> parse_cstr(size_t num_qubits, bool sign, const std::function<char(size_t)>& func) {
        ...
        for (size_t i = 0; i < num_qubits; i++) {
            ...
            switch (func(i)) {
                ...
                case 'I':
                    ...
                    break;
                default:
                    throw std::invalid_argument("can not parse character: " + std::to_string(func(i)));
            }
