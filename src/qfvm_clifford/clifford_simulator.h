...
struct circuit_simulator {
    ...
    // measurement.
    explicit circuit_simulator(size_t num_qubits, size_t seed = 42) : sim_tableau(num_qubits), sim_record(), rng() {
        rng.seed(seed);
    }

    // do a quantum gate except measurement gate
    template <size_t vec_size, size_t... S>
    result unpack_vector(auto f, tableau<word_size>& t, auto& vec, std::index_sequence<S...>) {
    ...
    result unpack_vector(auto f, tableau<word_size>& t, auto& vec) {
        ...
        return unpack_vector<vec_size>(f, t, vec, std::make_index_sequence<vec_size>());
    }

    // do a measurement gate
    template <size_t... S>
    result unpack_vector(auto f, tableau<word_size>& t, auto& vec, std::mt19937_64& rng, std::index_sequence<S...>) {
        // TODO: maybe can be better
        return std::get<gate_type::COLLAPSING_GATE>(f)(t, rng, vec[S]...);
    }

    template <size_t vec_size>
    result unpack_vector(auto f, tableau<word_size>& t, auto& vec, std::mt19937_64& rng) {
        if (vec.size() != vec_size)
            throw std::runtime_error("wrong number of qubits for gate");
    ...
    void do_circuit(const quantum_circuit& qc) {
        if (qc.max_qubit() >= sim_tableau.num_qubits) {
            ...
            std::cerr << "WARNING: expanding tableau to " << qc.max_qubit() << " qubits\n";
        }
    ...
    size_t record_size() {
        return sim_record.size();
    }

    // get the measurement record
    auto current_measurement_record() const {
        return sim_record;
    }
