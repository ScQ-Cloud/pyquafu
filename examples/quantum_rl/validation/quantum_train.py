...
parser.add_argument('--state_bounds',
                    type=np.array,
...
parser.add_argument('--lr_in',
                    type=float,
                    default=0.1,
                    help='learning rate of input parameter')
parser.add_argument('--lr_var',
                    type=float,
                    default=0.01,
                    help='learning rate of variational parameter')
parser.add_argument('--lr_out',
                    type=float,
                    default=0.1,
                    help='learning rate of output parameter')
...
parser.add_argument('--backend_quafu',
                    type=str,
                    default='ScQ-P10',
                    help='which quafu backend to use')
...
def main(qubits, genotype, observables):
    ...
    model = Network(qubits, genotype, args.n_actions, args.beta, observables,
                    args.env_name)
    model.load_weights(args.model_path)

    n_epochs = args.epochs

    optimizer_in = tf.keras.optimizers.Adam(learning_rate=args.lr_in,
                                            amsgrad=True)
    optimizer_var = tf.keras.optimizers.Adam(learning_rate=args.lr_var,
                                             amsgrad=True)
    optimizer_out = tf.keras.optimizers.Adam(learning_rate=args.lr_out,
                                             amsgrad=True)
    ...
    for epoch in range(n_epochs):
        ...
        reward_his = train(model, optimizer_in, optimizer_var, optimizer_out,
                           w_in, w_var, w_out)
...
# Training
def train(model, optimizer_in, optimizer_var, optimizer_out, w_in, w_var,
          w_out):
    ...
    for batch in range(args.n_episodes // args.batch_size):
        # Gather episodes
        tasklist, episodes = gather_episodes(args.state_bounds, args.n_actions,
                                             model, args.batch_size,
                                             args.env_name, args.beta,
                                             args.backend, args.backend_quafu,
                                             args.shots, args.n_qubits, qubits,
                                             genotype)
        ...
        if episode_reward_history[-1] >= best_reward:
            ...
            model.save_weights(
                os.path.join(
                    args.save,
                    'weights_id10_quafu_{}.h5'.format(int(best_reward))))
        ...
        logging.info('train finished episode: %f',
        ...
        model.save_weights(
            os.path.join(args.save, 'weights_id10_quafu_latest.h5'))
