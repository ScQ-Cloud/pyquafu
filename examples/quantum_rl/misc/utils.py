...
class Alternating_(tf.keras.layers.Layer):
    ...
    def __init__(self, obsw):
        ...
        self.w = tf.Variable(initial_value=tf.constant(obsw),
                             dtype="float32",
                             trainable=True,
                             name="obsw")
...
def gather_episodes(state_bounds,
                    n_actions,
                    model,
                    n_episodes,
                    env_name,
    ...
    while not all(done):
        ...
        if backend == 'cirq':
            ...
            circuit, _, _ = generate_circuit(qubits, genotype, newtheta,
                                             newlamda,
                                             states.numpy()[0])
            taskid, expectation = get_quafu_exp(circuit, n_qubits,
                                                backend_quafu, shots)
