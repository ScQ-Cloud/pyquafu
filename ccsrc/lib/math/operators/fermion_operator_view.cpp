...
std::vector<uint64_t> SingleFermionStr::NumOneMask(const compress_term_t& fermion) {
    ...
    std::transform(fermion.first.begin(), fermion.first.end(), out.begin(),
                   [&](auto& i) { return quafu::CountOne(i) & 1; });
    return out;
}
...
bool SingleFermionStr::InplaceMulCompressTerm(const term_t& term, compress_term_t& fermion) {
    ...
    auto one_mask_of_word = quafu::CountOne(static_cast<uint64_t>(word));
    ...
    if (ori_term.size() < group_id + 1) {
        ...
        one_mask = (one_mask
                    + (quafu::CountOne(static_cast<uint64_t>(word))
                       & quafu::CountOne(ori_term[group_id] & low_mask)))
                   & 1;
    }
    if (one_mask & 1) {
        fermion.second *= -1.0;
...
auto SingleFermionStr::Mul(const compress_term_t& lhs, const compress_term_t& rhs) -> compress_term_t {
    ...
    for (std::size_t i = 0; i < max_size; i++) {
        if (i < min_size) {
            total_one += one_in_low & quafu::CountOne(r_k[i]);
            one_in_low += quafu::CountOne(l_k[i]);
...
std::tuple<tn::Tensor, uint64_t> SingleFermionStr::MulSingleCompressTerm(uint64_t a, uint64_t b) {
    ...
    for (size_t b_i = b; b_i != 0; b_i >>= 3) {
        ...
        if (rhs != TermValue::I) {
            total_one += one_in_low & quafu::CountOne(b & 7);
        }
        one_in_low += quafu::CountOne(a & 7);
        ...
        for (auto word = k.rbegin(); word != k.rend(); ++word) {
            if ((*word) != 0) {
                n_qubits = std::max(n_qubits,
                                    static_cast<int>((63 - quafu::CountLeadingZero(*word)) / 3 + group_id * 21));
                break;
            }
            group_id -= 1;
        }
